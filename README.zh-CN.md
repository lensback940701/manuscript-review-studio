...
**打开 Windows 程序，选择模型，放入整篇论文，就能获得一次严格而有结论的独立复审——不需要安装 Codex，也不需要搭建任何 agent。**

Manuscript Review Studio 是一款真正面向作者、能够独立启动的桌面应用。它自带本地操作界面，可以直接读取你选择的稿件，并使用你自己的 API key 连接 DeepSeek、Kimi 或 Gemini。Windows 封装版运行时不依赖 Codex，也不依赖 ChatGPT，更不要求用户理解命令行或 agent 工具。
...
- **真正独立的 Windows 程序。** 双击启动本地界面；使用封装版不需要安装 Codex、配置命令行 agent 或准备开发环境。
- **针对整篇论文作判断。** 它审查的是全文论证与章节之间的关系，而不是只点评几个孤立段落。
- **审阅方式可以贴合你的场景。** 支持标准模式、审稿人格/严格度模式和目标期刊样本模式。
- **给出清楚的修改终点。** 输出有限且明确的结论、最重要的修改方向、应当保护的优点，以及单列的证据或投稿事项。
- **全文外发前由你决定。** 程序会显示文件、provider 与 model，并要求本次运行重新确认后才会发送稿件。

技术层面上，程序内部确实嵌入了多阶段审稿 harness，而不是只把提示词转交给 API。但这些机制的作用是让模型审得更严格、更一致；普通作者不需要理解或配置它们。
