...
公开仓库包含 Skill 指令、界面元数据、standalone 运行层、契约辅助程序、标准事项代码说明、合成测试材料、回归测试和五张由作者提供的说明插图。它不包含真实稿件、项目专用示例、证据包、内部 failure-first/build 回执、本地绝对路径、凭据、本地画图提示词或未采用的图片版本。

源码仓库不提交第三方数据集、稿件内容、外部模型输出或本地构建的 EXE；standalone 构建可能捆绑的依赖见[第三方说明](THIRD_PARTY_NOTICES.md)。软件与文档按照 Apache License 2.0 发布；仓库中的五张说明插图已在公开前单独核验。
