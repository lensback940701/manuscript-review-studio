...
EVALUATION_MODES = ("standard", "strictness", "journal_benchmark")
...
def validate_mode(mode: str | None) -> str:
    ...
    if m not in EVALUATION_MODES:
        raise ValueError(f"unknown evaluation mode: {mode!r}, expected one of {EVALUATION_MODES}")
    return m


def validate_strictness(level: str | None) -> str:
    s = (level or "moderate").strip().lower()
    if s not in STRICTNESS_LEVELS:
        raise ValueError(f"unknown strictness level: {level!r}, expected one of {STRICTNESS_LEVELS}")
    return s
...
def validate_cross_stage_consistency(coverage: Mapping[str, Any], model_state: Mapping[str, Any]) -> None:
    ...
    for cause in model_state["material_root_causes"]:
        dimension = _cause_dimension(cause)
        if dimension is not None and dimension not in COVERAGE_DIMENSIONS:
