...
"""
    ...
    mode_guidance = ""
    if mode == "journal_benchmark" and target_journal_name:
        samples_info = f"共 {benchmark_profile.sample_papers_count} 篇近期已发表同类论文" if benchmark_profile else "已配置样本论文库"
        mode_guidance = f"""
【审阅模式设定：目标期刊对齐模式（Target Journal Benchmark Mode）】
- 目标期刊：《{target_journal_name}》
- 期刊定位/Scope：{target_journal_scope or "未提供"}
- 参照样本库：{samples_info}

中文解读撰写特定要求：
1. 【status_explanation】：明确说明本次门禁裁决以《{target_journal_name}》顶刊水平与样本论文库为参照基准。若裁决为 REOPEN_SUBSTANTIVE_REVISION 或 ONE_BOUNDED_ROUND，必须明确指出与《{target_journal_name}》主流文章在理论对话深度（如是否充分展开核心学科范式对话）、实证颗粒度或方法严密性上的实质差距；若为 STOP_REVISING，说明文稿已满足该刊当前的修止边界。
2. 【judgment_basis】：必须将“以目标期刊《{target_journal_name}》及其已发表同类样本论文为基准参照系”明确列为首要依据之一。
3. 【assessment_dimensions】：在评述各重点考察维度时，必须直接对比《{target_journal_name}》的学科要求与样本论文范式（如理论深度是否达标、实证细节是否充分）。
4. 【selective_findings】与【optional_micro_adjustments】：针对《{target_journal_name}》的学科关切给出精准的章节观察与微调建议。
5. 【pre_submission_checklist】：包含针对《{target_journal_name}》投稿要求的特定核对清单。
"""
    elif mode == "strictness" and strictness_level == "strict":
        mode_guidance = """
【审阅模式设定：顶级期刊审稿人苛求模式（Strict / Top-tier Referee Mode）】
中文解读撰写特定要求：
1. 【status_explanation】与【assessment_dimensions】：从顶刊严苛审稿人视角出发，重点解析为何判定为当前裁决，指出潜在的方法学盲区、未经严格检验的假设或理论对话浅层化问题。
2. 【pre_submission_checklist】：增加针对严苛同行评审可能发难关键点的检查清单。
"""
...
def generate_interpretation(
    ...
    *,
    ...
    messages = build_interpretation_messages(
        ...
        target_journal_scope=target_journal_scope,
