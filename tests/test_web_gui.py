...
class WebGuiTests(unittest.TestCase):
    ...
    def test_check_samples_relevance_endpoint_success(self) -> None:
        relevance_json = json.dumps({
            "rating": "HIGH",
            "score": 0.88,
            "explanation": "样本与稿件高度契合",
            "methodological_alignment": "定性案例研究范式一致",
            "theoretical_alignment": "同属转型理论体系",
        ...
        with tempfile.TemporaryDirectory() as directory, patch.dict(
            os.environ, {"DEEPSEEK_API_KEY": "local"}, clear=True
        ), patch.object(
            ChatCompletionClient, "complete", return_value=mock_result
        ):
            manuscript = Path(directory) / "paper.md"
            manuscript.write_text(_long_manuscript(), encoding="utf-8")
            samples_dir = Path(directory) / "samples"
            samples_dir.mkdir()
            for i in range(5):
                (samples_dir / f"sample_{i}.md").write_text(_long_manuscript(), encoding="utf-8")

            status, body, _headers = self._request(
                "/api/check-samples-relevance",
                token=self.state.token,
                payload={
                    "manuscript_path": str(manuscript),
                    ...
                    "provider": "deepseek",
                    "model": "deepseek-v4-pro",
                    "target_journal_name": "Economic Geography",
                    "target_journal_scope": "Economic geography and transitions",
                },
            )
            self.assertEqual(200, status)
            data = json.loads(body.decode("utf-8"))
            self.assertEqual("HIGH", data["rating"])
            self.assertEqual(0.88, data["score"])
            self.assertTrue(data["is_suitable"])
            self.assertEqual(5, data["sample_papers_count"])
